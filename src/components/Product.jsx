...
export default function Product(productListAll, index) {
  const { eachProduct: product } = productListAll;
  const [productQuentity, setProductQuentity] = useState(0);
  const totalProduct = (productQuentity) => {
    ...
    <div className="row align-items-center pb-3 ps-4 pe-4">
      ...
      <div className="col-md-3" style={quantityButtonStyle}>
        <div className="btn-group" role="group" aria-label="Basic example">
          ...
          <Cart onProductNumber={totalProduct} />
        </div>
      </div>
      <div className="col-md-3 p-0" style={quantityButtonStyle}>
        <button type="button" className="btn btn-warning w-100">
          ৳ {productQuentity * product.Price}
        </button>
      </div>
